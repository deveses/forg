...
void EToolWindow::OnClose() { ShowWindow(SW_HIDE); }
...
void EToolWindow::UndockWindow()
{
    ...
    SetWindowPos(HWND_TOP, m_rtBeforeDock.left, m_rtBeforeDock.top,
                 ...
                 m_rtBeforeDock.bottom - m_rtBeforeDock.top, SWP_FRAMECHANGED);
    ShowWindow(SW_SHOW);
    m_isDocked = FALSE;
}

void EToolWindow::SetDockMode(int nMode) { m_dockMode = nMode; }

DWORD EToolWindow::Create(LPCTSTR lpWindowName, int x, int y, int nWidth,
                          int nHeight, DWORD dwStyle, HWND hWndParent)
{
    return (EWnd::Create(_T("EToolWindow"), lpWindowName, x, y, nWidth, nHeight,
                         ...
                         WS_EX_DLGMODALFRAME | WS_EX_TOOLWINDOW, hWndParent));
}
