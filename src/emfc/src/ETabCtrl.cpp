...
DWORD ETabCtrl::Create(LPCTSTR lpWindowName, int x, int y, int nWidth,
                       int nHeight, DWORD dwStyle, HWND hWndParent)
{
    return EWnd::Create(WC_TABCONTROL, lpWindowName, x, y, nWidth, nHeight,
                        WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | dwStyle, 0,
                        hWndParent);
}
...
int ETabCtrl::GetCurSel() { return TabCtrl_GetCurSel(m_hWnd); }
...
BOOL ETabCtrl::DeleteAllItems() { return TabCtrl_DeleteAllItems(m_hWnd); }
...
HIMAGELIST ETabCtrl::GetImageList() { return TabCtrl_GetImageList(m_hWnd); }
...
HWND ETabCtrl::GetToolTips() { return TabCtrl_GetToolTips(m_hWnd); }
