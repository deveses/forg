...
LRESULT CALLBACK EWindowProc(HWND hWnd,     // handle to window
...
)
{
    ...
    if (ew != NULL)
        return ew->WindowProc(uMsg, wParam, lParam);
    else
    {
        switch (uMsg)
        {
...
DWORD EWnd::Create(LPCTSTR lpszClassName, LPCTSTR lpWindowName, int x, int y,
                   int nWidth, int nHeight, DWORD dwStyle, DWORD dwExStyle,
                   HWND hWndParent)
{
    WNDCLASSEX wc;
        ...
        !GetClassInfoEx(NULL, lpszClassName, &wc))
    {
        ...
        wc.hbrBackground =
            /*CreateSolidBrush(RGB(192,192,192));*/ GetSysColorBrush(
                COLOR_3DFACE); //(HBRUSH)GetStockObject(LTGRAY_BRUSH);
    ...
    m_hWnd =
        CreateWindowEx(dwExStyle, lpszClassName, lpWindowName, dwStyle, x, y,
...
BOOL EWnd::ShowWindow(int nCmdShow) { return ::ShowWindow(m_hWnd, nCmdShow); }
...
LRESULT EWnd::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        ...
        break;
    case WM_MOUSEWHEEL:
        OnMouseWheel(wParam, MAKEPOINTS(lParam),
    ...
    case WM_NOTIFY:
        if (!OnNotify(wParam, lParam, &m_lastMessage.lResult))
            DefWindowProc(m_lastMessage.uMsg, m_lastMessage.wParam,
                          m_lastMessage.lParam);
        break;
    case WM_KEYDOWN:
    ...
    default:
        return DefWindowProc(m_lastMessage.uMsg, m_lastMessage.wParam,
                             m_lastMessage.lParam);
    }
...
inline HWND EWnd::GetSafeHwnd() const { return this == NULL ? NULL : m_hWnd; }
...
HWND EWnd::SetFocus() { return (::SetFocus(m_hWnd)); }
...
BOOL EWnd::SetMenu(HMENU hMenu) { return (::SetMenu(m_hWnd, hMenu)); }
...
HICON EWnd::SetIcon(HICON hIcon, BOOL bBigIcon)
{
    return (HICON)SendMessage(
        m_hWnd, WM_SETICON, (bBigIcon ? ICON_BIG : ICON_SMALL), (LPARAM)hIcon);
}

BOOL EWnd::SetWindowPos(HWND pWndInsertAfter, int x, int y, int cx, int cy,
                        UINT nFlags)
{
...
HWND EWnd::SetCapture() { return ::SetCapture(m_hWnd); }

HWND EWnd::GetCapture() { return ::GetCapture(); }

HMENU EWnd::GetMenu() { return ::GetMenu(m_hWnd); }

HDC EWnd::GetWindowDC() { return (::GetWindowDC(m_hWnd)); }

HDC EWnd::GetDC() { return ::GetDC(m_hWnd); }

LONG EWnd::GetWindowLong(int nIndex) { return ::GetWindowLong(m_hWnd, nIndex); }
...
void EWnd::GetWindowRect(LPRECT lpRect) { ::GetWindowRect(m_hWnd, lpRect); }

void EWnd::GetClientRect(LPRECT lpRect) { ::GetClientRect(m_hWnd, lpRect); }

HWND EWnd::GetDlgItem(int nID) const { return ::GetDlgItem(m_hWnd, nID); }
...
void EWnd::Invalidate(BOOL bErase) { ::InvalidateRect(m_hWnd, NULL, bErase); }
...
BOOL EWnd::OnCommand(WPARAM wParam, LPARAM lParam) { return FALSE; }

HBRUSH EWnd::OnCtlColor(HDC pDC, HWND pWnd, UINT nCtlColor)
{
    return (HBRUSH)DefWindowProc(m_lastMessage.uMsg, m_lastMessage.wParam,
                                 m_lastMessage.lParam);
}
...
BOOL EWnd::ExecuteDlgInit(LPCTSTR lpszResourceName)
{
    ...
    if (lpszResourceName != NULL)
    {
        ...
        HINSTANCE hInst = GetModuleHandle(
            NULL); // AfxFindResourceHandle(lpszResourceName, RT_DLGINIT);
...
BOOL EWnd::ExecuteDlgInit(LPVOID lpResource)
{
  ...
  //#ifdef _AFX_NO_OCC_SUPPORT
           ...
           // catch some.
           if (nMsg == LB_ADDSTRING || nMsg == CB_ADDSTRING || nMsg ==
  CBEM_INSERTITEM) ASSERT(*((LPBYTE)lpnRes + (UINT)dwLen - 1) == 0); #endif
           ...
           if (nMsg == CBEM_INSERTITEM)
           {
              ...
              if (::SendDlgItemMessage(m_hWnd, nIDC, nMsg, 0, (LPARAM) &item) ==
  -1) bSuccess = FALSE;
           }
  ...
  #endif // !_AFX_NO_OCC_SUPPORT
           {
              ...
              if (::SendDlgItemMessageA(m_hWnd, nIDC, nMsg, 0, (LPARAM) lpnRes)
  == -1) bSuccess = FALSE;
           }
...
void EWnd::SendMessageToDescendants(UINT message, WPARAM wParam, LPARAM lParam,
...
BOOL EWnd::PreTranslateMessage(MSG* pMsg) { return FALSE; }
...
DWORD EWnd::GetStyle() { return GetWindowLong(GWL_STYLE); }

int EWnd::RunModalLoop(DWORD dwFlags)
{
    if (m_hWnd == NULL || (m_nFlags & WF_MODALLOOP))
        return -1; // window must be created
...
BOOL EWnd::ContinueModal() { return m_nFlags & WF_CONTINUEMODAL; }
...
void EWnd::UpdateWindow() { ::UpdateWindow(m_hWnd); }
...
void EMFC_API DDX_Text(EDataExchange* pDX, int nIDC, LPTSTR value, int nMaxLen)
{
    ...
    if (pDX->m_bSaveAndValidate)
    {
        ...
        // if (nLen > nRetrieved)
        //    TRACE1("Text in control ID %d is too long. Call
        //    DDV_MaxChars()!\n", nIDC);
    }
    else
    {
