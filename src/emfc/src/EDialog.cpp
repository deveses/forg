...
INT_PTR CALLBACK EDialog::EDialogProc(HWND hwndDlg,  // handle to dialog box
                                      UINT uMsg,     // message
...
BOOL EDialog::Create(LPCTSTR lpszTemplateName, HWND pParentWnd)
{
    ...
    if (!CreateIndirect(hDialogTemplate, pParentWnd, m_hInstance))
        return FALSE;
...
BOOL EDialog::CreateIndirect(HGLOBAL hDialogTemplate, HWND pParentWnd,
                             HINSTANCE hInst)
{
...
BOOL EDialog::CreateIndirect(LPCDLGTEMPLATE lpDialogTemplate, HWND pParentWnd,
                             HINSTANCE hInst)
{
    if (lpDialogTemplate == NULL /* || hInst==NULL*/)
        return FALSE;
    m_hWnd = ::CreateDialogIndirectParam(hInst, lpDialogTemplate, pParentWnd,
                                         EDialogProc, (LPARAM)this);
    m_nModalResult = -1;
...
int EDialog::DoModal()
{
    ...
    hInst =
        NULL; // AfxGetResourceHandle(); //m_hInstance; //GetModuleHandle(NULL);
    ...
    if (CreateIndirect(lpDialogTemplate, m_hWndParent, hInst))
    {
...
void EDialog::OnCancel() { EndDialog(IDCANCEL); }
