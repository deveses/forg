...
BOOL EAsyncSocket::Create(UINT nSocketPort, int nSocketType, long IEvent,
...
BOOL EAsyncSocket::Socket(int nSocketType, long lEvent, int nProtocolType,
                          int nAddressFormat)
{
...
{
    if (connect(m_hSocket, lpSockAddr, nSockAddrLen) == 0)
        return TRUE;
...
BOOL EAsyncSocket::GetPeerName(LPTSTR& rPeerAddress, UINT& rPeerPort)
{
    ...
    BOOL bResult =
        (getpeername(m_hSocket, (SOCKADDR*)&sockAddr, &nSockAddrLen) == 0);
    if (bResult)
    {
...
BOOL EAsyncSocket::GetSockName(LPTSTR& rSocketAddress, UINT& rSocketPort)
{
    ...
    int nSockAddrLen = sizeof(sockAddr);
    BOOL bResult =
        (getsockname(m_hSocket, (SOCKADDR*)&sockAddr, &nSockAddrLen) == 0);
    if (bResult)
    {
...
BOOL EAsyncSocket::Accept(EAsyncSocket& rConnectedSocket, SOCKADDR* lpSockAddr,
                          int* lpSockAddrLen)
{
...
{
    return WSAAsyncSelect(m_hSocket, m_hSocketWnd, WM_SOCKET_NOTIFY, lEvent) !=
           SOCKET_ERROR;
}
