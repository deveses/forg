...
DWORD EToolBarCtrl::Create(DWORD dwStyle, int x, int y, int nWidth, int nHeight,
                           HWND pParentWnd)
{
    DWORD ret = EWnd::Create(TOOLBARCLASSNAME, _T(""), x, y, nWidth, nHeight,
                             WS_CHILD | dwStyle, WS_EX_TOOLWINDOW, pParentWnd);
...
{
    return (int)SendMessage(m_hWnd, TB_ADDSTRING, (WPARAM)GetModuleHandle(NULL),
                            (LPARAM)MAKELONG(idString, 0));
}

int EToolBarCtrl::AddStrings(LPCTSTR lpszStrings)
{
    return (int)SendMessage(m_hWnd, TB_ADDSTRING, (WPARAM)0,
                            (LPARAM)lpszStrings);
}

BOOL EToolBarCtrl::AddButtons(int nNumButtons, LPTBBUTTON lpButtons)
{
    return (BOOL)SendMessage(m_hWnd, TB_ADDBUTTONS, (WPARAM)(UINT)nNumButtons,
                             (LPARAM)lpButtons);
}

void EToolBarCtrl::AutoSize() { SendMessage(m_hWnd, TB_AUTOSIZE, 0, 0); }
...
{
    return (HIMAGELIST)SendMessage(m_hWnd, TB_SETIMAGELIST, 0,
                                   (LPARAM)(HIMAGELIST)himlNew);
}

BOOL EToolBarCtrl::SetButtonWidth(int cxMin, int cxMax)
{
    return (BOOL)SendMessage(m_hWnd, TB_SETBUTTONWIDTH, 0,
                             (LPARAM)(DWORD)MAKELONG(cxMin, cxMax));
}

BOOL EToolBarCtrl::SetMaxTextRows(int iMaxRows)
{
    return (BOOL)SendMessage(m_hWnd, TB_SETMAXTEXTROWS, (WPARAM)(INT)iMaxRows,
                             0);
}

DWORD EToolBarCtrl::SetExtendedStyle(DWORD dwExStyle)
{
    return (DWORD)SendMessage(m_hWnd, TB_SETEXTENDEDSTYLE, 0,
                              (LPARAM)dwExStyle);
}
...
{
    return (BOOL)SendMessage(m_hWnd, TB_SETBUTTONINFO, (WPARAM)(INT)nID,
                             (LPARAM)(LPTBBUTTONINFO)ptbbi);
}

BOOL EToolBarCtrl::GetItemRect(int nIndex, LPRECT lpRect)
{
    return (BOOL)SendMessage(m_hWnd, TB_GETITEMRECT, (WPARAM)nIndex,
...
{
    return (BOOL)SendMessage(m_hWnd, TB_INSERTBUTTON, (WPARAM)nIndex,
                             (LPARAM)(LPTBBUTTON)lpButton);
}
...
{
    return (BOOL)SendMessage(m_hWnd, TB_CHANGEBITMAP, (WPARAM)idButton,
                             (LPARAM)MAKELPARAM(iBitmap, 0));
}

BOOL EToolBarCtrl::CheckButton(int nID, BOOL bCheck)
{
    return (BOOL)SendMessage(m_hWnd, TB_CHECKBUTTON, (WPARAM)nID,
                             (LPARAM)MAKELONG(bCheck, 0));
}

void EToolBarCtrl::Customize() { SendMessage(m_hWnd, TB_CUSTOMIZE, 0, 0); }
...
{
    return (BOOL)SendMessage(m_hWnd, TB_ENABLEBUTTON, (WPARAM)nID,
...
{
    return (BOOL)SendMessage(m_hWnd, TB_HIDEBUTTON, (WPARAM)nID,
                             (LPARAM)MAKELONG(bHide, 0));
}

BOOL EToolBarCtrl::Indeterminate(int nID, BOOL bIndeterminate)
{
    return (BOOL)SendMessage(m_hWnd, TB_INDETERMINATE, (WPARAM)nID,
                             (LPARAM)MAKELONG(bIndeterminate, 0));
}

BOOL EToolBarCtrl::PressButton(int nID, BOOL bPress)
{
    return (BOOL)SendMessage(m_hWnd, TB_PRESSBUTTON, (WPARAM)nID,
                             (LPARAM)MAKELONG(bPress, 0));
}

BOOL EToolBarCtrl::SetAnchorHighlight(BOOL fAnchor)
{
    return (BOOL)SendMessage(m_hWnd, TB_SETANCHORHIGHLIGHT,
                             (WPARAM)(BOOL)fAnchor, 0);
}

BOOL EToolBarCtrl::SetCmdID(int nIndex, UINT nID)
{
    return (BOOL)SendMessage(m_hWnd, TB_SETCMDID, (WPARAM)(UINT)nIndex,
                             (LPARAM)(UINT)nID);
}
...
{
    return (BOOL)SendMessage(m_hWnd, TB_SETSTATE, (WPARAM)nID,
                             (LPARAM)MAKELONG(nState, 0));
}
...
BOOL EToolBarCtrl::SetBitmapSize(int dxBitmap, int dyBitmap)
{
    return (BOOL)SendMessage(m_hWnd, TB_SETBITMAPSIZE, 0,
                             (LPARAM)MAKELONG(dxBitmap, dyBitmap));
}
