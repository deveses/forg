...
#define WM_SETMESSAGESTRING                                                    \
    0x0362                        // wParam = nIDS (or 0),
                                  // lParam = lpszOther (or NULL)
#define WM_IDLEUPDATECMDUI 0x0363 // wParam == bDisableIfNoHandler
#define WM_INITIALUPDATE 0x0364   // (params unused) - sent to children
#define WM_COMMANDHELP                                                         \
    0x0365 // lResult = TRUE/FALSE,
           // lParam = dwContext
#define WM_HELPHITTEST                                                         \
    0x0366                     // lResult = dwContext,
                               // lParam = MAKELONG(x,y)
#define WM_EXITHELPMODE 0x0367 // (params unused)
#define WM_RECALCPARENT                                                        \
    0x0368 // force RecalcLayout on frame window
           //  (only for inplace frame windows)
#define WM_SIZECHILD                                                           \
    0x0369                       // special notify from COleResizeBar
...
#define WM_DISABLEMODAL                                                        \
    0x036C                    // lResult = 0, disable during modal state
...
#define WM_ACTIVATETOPLEVEL                                                    \
    0x036E // wParam = nState (like WM_ACTIVATE)
...
#define WF_KEEPMINIACTIVE                                                      \
    0x0200 // stay activate even though you are deactivated
...
class EMFC_API EWnd
{
  ...
  public:
    ...
    BOOL SetWindowPos(HWND pWndInsertAfter, int x, int y, int cx, int cy,
                      UINT nFlags);
    HDC GetWindowDC();
    HWND SetFocus();
    ...
    DWORD Create(LPCTSTR lpszClassName, LPCTSTR lpWindowName, int x, int y,
