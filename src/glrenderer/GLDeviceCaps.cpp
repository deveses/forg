...
#define GLCAPS_CHECK_AND_SETUP(str, ext, cap)                                  \
    {                                                                          \
        if (!m_capabilities[cap] &&                                            \
            str == (StringTokenizer<char>::string_type)_T(#ext))               \
        {                                                                      \
...
GLDeviceCaps::GLDeviceCaps(void) : m_capabilities(GLCaps_Count, false) {}
...
int GLDeviceCaps::ReadExtensions()
{
    ...
    while (tokenizer.HasMoreTokens())
    {
        ...
        GLCAPS_CHECK_AND_SETUP(ext, GL_EXT_separate_specular_color,
                               GLCaps_SeparateSpecularColor);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_multitexture, GLCaps_Multitexture);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_texture_compression,
                               GLCaps_TextureCompression);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_texture_cube_map,
                               GLCaps_TextureCubeMap);
        GLCAPS_CHECK_AND_SETUP(ext, GL_SGIS_generate_mipmap,
                               GLCaps_GenerateMipmap);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_depth_texture, GLCaps_DepthTexture);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_vertex_buffer_object,
                               GLCaps_VertexBufferObject);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_vertex_program,
                               GLCaps_VertexProgram);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_texture_non_power_of_two,
                               GLCaps_TextureNonPowerOfTwo);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_shader_objects,
                               GLCaps_ShaderObjects);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_shading_language_100,
                               GLCaps_ShadingLanguage100);
        GLCAPS_CHECK_AND_SETUP(ext, GL_ARB_pixel_buffer_object,
