...
class GLRenderer : public IRenderer
{
  public:
    ...
    GLRenderDevice* CreateDevice(HWIN hWindow,
                         ...
                         RENDER_PARAMETERS* pPresentationParameters)
{
    ...
    //	if (! SetClassLongPtr(hWindow, GCL_STYLE, CS_HREDRAW | CS_VREDRAW |
    // CS_OWNDC)) 		return false;
    HDC hDC = GetDC(hWin);
    if (hDC == NULL)
        return 0;
    ...
    for (int i = 1; i <= numFormats; i++)
    {
        ...
        DBG_MSG("%d: bpp: %d, depth: %d, opengl: %d, windowed: %d, "
                "accelerated: %d\n",
        ...
        //    q=q+0x2000; if (wacc==-1 || (wacc==0 && soft) || (wacc==1 && (mcd
        //    || icd))) q=q+0x1000; if (mcd || icd) q=q+0x0040; if (icd)
        //    q=q+0x0002; if (wbpp==-1 || (wbpp==bpp)) q=q+0x0800; if (bpp>=16)
        //    q=q+0x0020; if (bpp==16) q=q+0x0008; if (wdepth==-1 ||
        //    (wdepth==depth)) q=q+0x0400; if (depth>=16) q=q+0x0010; if
        //    (depth==16) q=q+0x0004; if (!pal) q=q+0x0080; if (bitmap)
        //    q=q+0x0001; if (q>maxqual) {maxqual=q; maxindex=i;max_bpp=bpp;
        //    max_depth=depth; max_dbl=dbuff?1:0; max_acc=soft?0:1;}
    }
