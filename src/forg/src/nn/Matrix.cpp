...
Matrix MatrixMLP::Forward(const Matrix& input) const
{
    ...
    {
        for (std::size_t hidden_index = 0; hidden_index < m_hidden_count;
             ++hidden_index)
        {
            double value = m_b1[hidden_index];
            for (std::size_t input_index = 0; input_index < m_input_count;
                 ++input_index)
            {
                value +=
                    input(row, input_index) * m_w1(hidden_index, input_index);
            }
    ...
    for (std::size_t row = 0; row < input.Rows(); ++row)
    {
        for (std::size_t output_index = 0; output_index < m_output_count;
             ++output_index)
        {
                             ...
                             double learning_rate)
{
    ...
    for (std::size_t row = 0; row < input.Rows(); ++row)
    {
        if (labels[row] >= m_output_count)
            return 0.0;

        double max_logit = logits(row, 0);
        for (std::size_t output_index = 1; output_index < m_output_count;
             ++output_index)
        {
        ...
        loss += -std::log(std::max(grad_logits(row, labels[row]),
    ...
    Matrix grad_hidden(input.Rows(), m_hidden_count);
    for (std::size_t row = 0; row < input.Rows(); ++row)
    {
        for (std::size_t output_index = 0; output_index < m_output_count;
             ++output_index)
        {
            const double grad = grad_logits(row, output_index);
            grad_b2[output_index] += grad;
            for (std::size_t hidden_index = 0; hidden_index < m_hidden_count;
                 ++hidden_index)
            {
                grad_w2(output_index, hidden_index) +=
                    grad * hidden(row, hidden_index);
                grad_hidden(row, hidden_index) +=
                    grad * m_w2(output_index, hidden_index);
            }
        }
    }
    ...
    for (std::size_t row = 0; row < input.Rows(); ++row)
    {
        for (std::size_t hidden_index = 0; hidden_index < m_hidden_count;
             ++hidden_index)
        {
            const double grad = z1(row, hidden_index) > 0.0
                                    ? grad_hidden(row, hidden_index)
                                    : 0.0;
            grad_b1[hidden_index] += grad;
            for (std::size_t input_index = 0; input_index < m_input_count;
                 ++input_index)
            {
                grad_w1(hidden_index, input_index) +=
                    grad * input(row, input_index);
            }
        }
    }
