...
#define DDSD_PITCH                                                             \
    0x00000008l // Required when pitch is provided for an uncompressed texture.
...
#define DDSD_LINEARSIZE                                                        \
    0x00080000l // Required when pitch is provided for a compressed texture.
...
#define GET_BITS(value, bit_index, bit_count)                                  \
    ((value) >> (bit_index) & ((1 << (bit_count)) - 1))
...
inline void endian_swap(unsigned short& x) { x = (x >> 8) | (x << 8); }
...
void DecodeDXT1(Color4b* _output, uint width, uint height, char* _input)
{
    ...
    for (uint v = 0; v < vblocks; v++)
    {
        for (uint h = 0; h < hblocks; h++)
        {
            ...
            {
                color_table[2].r =
                    ((int)color_table[0].r + color_table[1].r) / 2;
                color_table[2].g =
                    ((int)color_table[0].g + color_table[1].g) / 2;
                color_table[2].b =
                    ((int)color_table[0].b + color_table[1].b) / 2;
                color_table[3] = 0;
            }
            else
            {
                color_table[2].r =
                    (((int)color_table[0].r * 2) + (int)color_table[1].r) / 3;
                color_table[2].g =
                    (((int)color_table[0].g * 2) + (int)color_table[1].g) / 3;
                color_table[2].b =
                    (((int)color_table[0].b * 2) + (int)color_table[1].b) / 3;
                color_table[2].a = 0xff;

                color_table[3].r =
                    ((int)color_table[0].r + ((int)color_table[1].r * 2)) / 3;
                color_table[3].g =
                    ((int)color_table[0].g + ((int)color_table[1].g * 2)) / 3;
                color_table[3].b =
                    ((int)color_table[0].b + ((int)color_table[1].b * 2)) / 3;
...
Color4b* LoadDds(const char* filename, ImageDescription* bmp_info)
{
    ...
    else if (pSurfDesc->ddpf.dwFlags & DDPF_RGB)
    {
        ...
        for (uint i = 0; i < num_pixels; i++)
        {
            ...
            aBitmapBits[i].g =
                byte((pix & pSurfDesc->ddpf.dwGBitMask) >> off_g);
            aBitmapBits[i].r =
                byte((pix & pSurfDesc->ddpf.dwBBitMask) >> off_b);

            if (pSurfDesc->ddpf.dwFlags & DDPF_ALPHAPIXELS)
            {
                aBitmapBits[i].a =
                    byte((pix & pSurfDesc->ddpf.dwRGBAlphaBitMask) >> off_a);
                ;
            }
            else
            {
