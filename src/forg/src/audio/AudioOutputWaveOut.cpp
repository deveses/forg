...
class AudioOutputWaveOut : public IAudioOutput
{
  ...
  public:
    ...
    static void CALLBACK CallbackProc(HWAVEOUT hwo, UINT uMsg,
...
IAudioOutput* CreateAudioOutputWaveOut() { return new AudioOutputWaveOut(); }

///////////////////////////////////////////////////////////////////////////////

AudioOutputWaveOut::AudioOutputWaveOut() : m_device(NULL)
{
    m_header[0].lpData = 0;
...
AudioOutputWaveOut::~AudioOutputWaveOut() { Close(); }
...
bool AudioOutputWaveOut::Init()
{
    ...
    if (MMSYSERR_NOERROR == waveOutOpen(&m_device, WAVE_MAPPER, &fmt,
                                        (DWORD_PTR)CallbackProc,
                                        (DWORD_PTR)this, CALLBACK_FUNCTION))
    {
        ...
        if (MMSYSERR_NOERROR !=
            waveOutPrepareHeader(m_device, &m_header[1], sizeof(WAVEHDR)))
        {
            DBG_MSG("waveOutPrepareHeader() failed.\n");
        }
...
{
    if ((m_header[m_next_buffer].dwFlags != WHDR_PREPARED) &&
        ((m_header[m_next_buffer].dwFlags & WHDR_DONE) != WHDR_DONE))
    {
        return false;
    }
...
void AudioOutputWaveOut::Write(char* data, unsigned int size)
{
    if (CanWrite())
    {
        ...
        MMRESULT res =
            waveOutWrite(m_device, &m_header[m_next_buffer], sizeof(WAVEHDR));
