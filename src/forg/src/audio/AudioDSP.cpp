...
vector4_out coeffsB )
{
    // BPF:        H(s) = s / (s^2 + s/Q + 1)  (constant skirt gain, peak gain =
Q)
    //
    //                 b0 =   sin(w0)/2  =   Q*alpha
    //                 b1 =   0
...
void CalcHighpassCoeffs( float Fc, float Q, vector4_out coeffsA, vector4_out
coeffsB )
{
    Q = (float)native_fsel( Q, Q, .0001f );
    Fc = (float)native_fsel( Fc, Fc, FLT_EPSILON * 2.0f );
...
float native_fsel(float A, float B, float C) { return (A >= 0.0f ? B : C); }

//--------------------------------------------------------------------------------------
// Filter
//
// This is a standard "Direct Form 1" biquad filter implementation, which can be
// used to represent several different types of filter depending on the
// coefficients used. Helpers to generate a few different filter types are
// included.
//
// Biquad filters are IIR, or Infinite Impulse Response, designs. In practice
// this means that the filter uses a feedback mechanism: each calculation
// requires not only an input sample but also a number of previous samples from
// both the input and the output. The particular design used here is optimal
// because it requires exactly four output samples, which means that the output
// can be recycled one vector at a time.
