...
float Vector3::operator*(const Vector3& value) const { return Dot(value); }
...
void Vector3::Substract(Vector3& vOut, const Vector3& vLeft,
...
void Vector3::Scale(float fScale) { *this *= fScale; }
...
Vector3& Vector3::TransformCoordinate(Vector3& vOut, const Vector3& vSource,
                                      const Matrix4& mTransformation)
{
    Vector3 v(
        vSource.X * mTransformation.M11 + vSource.Y * mTransformation.M21 +
            vSource.Z * mTransformation.M31 + mTransformation.M41,
        vSource.X * mTransformation.M12 + vSource.Y * mTransformation.M22 +
            vSource.Z * mTransformation.M32 + mTransformation.M42,
        vSource.X * mTransformation.M13 + vSource.Y * mTransformation.M23 +
            vSource.Z * mTransformation.M33 + mTransformation.M43);
...
Vector3& Vector3::TransformNormal(Vector3& vOut, const Vector3& vSource,
                                  const Matrix4& mTransformation)
{
    Vector3 v(
        vSource.X * mTransformation.M11 + vSource.Y * mTransformation.M21 +
            vSource.Z * mTransformation.M31,
        vSource.X * mTransformation.M12 + vSource.Y * mTransformation.M22 +
            vSource.Z * mTransformation.M32,
        vSource.X * mTransformation.M13 + vSource.Y * mTransformation.M23 +
            vSource.Z * mTransformation.M33);

    return (vOut = v);
}

Vector3& Vector3::CatmullRom(Vector3& vOut, const Vector3& v0,
                             const Vector3& v1, const Vector3& v2,
                             const Vector3& v3, float s)
{
    // Q(s) = [(-s3 + 2s2 - s)p1 + (3s3 - 5s2 + 2)p2 + (-3s3 + 4s2 + s)p3 + (s3
    // - s2)p4] / 2;
