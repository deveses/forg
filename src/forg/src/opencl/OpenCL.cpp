...
typedef cl_context (*PFNCLCREATECONTEXT)(
...
/* Command Queue APIs */
typedef cl_command_queue (*PFNCLCREATECOMMANDQUEUE)(
    cl_context /* context */, cl_device_id /* device */,
    cl_command_queue_properties /* properties */, cl_int* /* errcode_ret */);

typedef cl_int (*PFNCLRELEASECOMMANDQUEUE)(
...
typedef cl_int (*PFNCLGETPROGRAMBUILDINFO)(
...
typedef cl_int (*PFNCLBUILDPROGRAM)(
...
typedef cl_int (*PFNCLGETKERNELWORKGROUPINFO)(
...
typedef cl_int (*PFNCLENQUEUEWRITEIMAGE)(
    cl_command_queue /* command_queue */, cl_mem /* image */,
...
typedef cl_int (*PFNCLENQUEUENDRANGEKERNEL)(
...
#define OPENCL_GETPROCADDRESS(Type, Name)                                      \
    OpenCL::Name = (OpenCL::Type)GetProcAddress(libocl, #Name);
bool LoadOpenCL()
{
    ...
    OPENCL_GETPROCADDRESS(PFNCLCREATEPROGRAMWITHSOURCE,
    ...
    OPENCL_GETPROCADDRESS(PFNCLGETKERNELWORKGROUPINFO,
...
bool CLPlatform::Initialize(cl_platform_id platform_id)
{
    ...
    CLV(OpenCL::clGetDeviceIDs(m_platform_id, CL_DEVICE_TYPE_ALL, 0, nullptr,
                               &deviceIdCount));
...
void CLPlatform::PrintInfo()
{
    ...
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_NAME, string_size,
                              platform_info.data(), &string_size);
    DBG_MSG("[OpenCL] Platform: %s\n", platform_info.data());

    // version
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_VERSION, 0, nullptr,
                              &string_size);
    platform_info.resize(string_size);
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_VERSION, string_size,
                              platform_info.data(), &string_size);
    DBG_MSG("[OpenCL] Version: %s\n", platform_info.data());

    // vendor
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_VENDOR, 0, nullptr,
                              &string_size);
    platform_info.resize(string_size);
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_VENDOR, string_size,
                              platform_info.data(), &string_size);
    DBG_MSG("[OpenCL] Vendor: %s\n", platform_info.data());

    // profile
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_PROFILE, 0, nullptr,
                              &string_size);
    platform_info.resize(string_size);
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_PROFILE, string_size,
                              platform_info.data(), &string_size);
    DBG_MSG("[OpenCL] Profile: %s\n", platform_info.data());

    // extensions
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_EXTENSIONS, 0, nullptr,
                              &string_size);
    platform_info.resize(string_size);
    OpenCL::clGetPlatformInfo(m_platform_id, CL_PLATFORM_EXTENSIONS,
                              string_size, platform_info.data(), &string_size);
...
#define CL_GETDEVICEINFO_PRIMITIVE(param, type)                                \
    {                                                                          \
        type value;                                                            \
        OpenCL::clGetDeviceInfo(m_device_id, param, sizeof(value), &value,     \
                                nullptr);                                      \
...
bool CLDevice::Initialize(cl_device_id device_id)
{
    ...
    // Get device info
    {
        ...
        OpenCL::clGetDeviceInfo(m_device_id, CL_DEVICE_NAME, string_size,
                                device_info.data(), &string_size);
        DBG_MSG("[OpenCL] [Device] Nane: %s\n", device_info.data());

        // Device vendor
        OpenCL::clGetDeviceInfo(m_device_id, CL_DEVICE_VENDOR, 0, nullptr,
                                &string_size);
        device_info.resize(string_size);
        OpenCL::clGetDeviceInfo(m_device_id, CL_DEVICE_VENDOR, string_size,
                                device_info.data(), &string_size);
        DBG_MSG("[OpenCL] [Device] Vendor: %s\n", device_info.data());

        // Device version
        OpenCL::clGetDeviceInfo(m_device_id, CL_DEVICE_VERSION, 0, nullptr,
                                &string_size);
        device_info.resize(string_size);
        OpenCL::clGetDeviceInfo(m_device_id, CL_DEVICE_VERSION, string_size,
                                device_info.data(), &string_size);
        DBG_MSG("[OpenCL] [Device] Version: %s\n", device_info.data());

        // Driver version
        OpenCL::clGetDeviceInfo(m_device_id, CL_DRIVER_VERSION, 0, nullptr,
                                &string_size);
        device_info.resize(string_size);
        OpenCL::clGetDeviceInfo(m_device_id, CL_DRIVER_VERSION, string_size,
                                device_info.data(), &string_size);
        DBG_MSG("[OpenCL] [Device] Driver version: %s\n", device_info.data());

        // Device type
        char* dev_type_name[] = {"CL_DEVICE_TYPE_DEFAULT", "CL_DEVICE_TYPE_CPU",
                                 "CL_DEVICE_TYPE_GPU",
        ...
        CL_GETDEVICEINFO_PRIMITIVEL(CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,
                                    cl_ulong);
...
bool CLContext::Create(cl_platform_id platform_id, cl_device_id device_id)
{
    ...
    const cl_context_properties contextProperties[] = {
        CL_CONTEXT_PLATFORM,
        reinterpret_cast<cl_context_properties>(platform_id), 0, 0};

    m_context = OpenCL::clCreateContext(contextProperties, 1, &device_id,
                                        nullptr, nullptr, &m_error);
...
bool CLContext::Create(cl_platform_id platform_id, cl_device_id* device_ids,
                       cl_uint count)
{
    // create context
    const cl_context_properties contextProperties[] = {
    ...
    m_context = OpenCL::clCreateContext(contextProperties, count, device_ids,
                                        nullptr, nullptr, &m_error);
...
bool CLProgram::CreateProgramWithSource(cl_context context, cl_uint count,
                                        const char** strings,
                                        const size_t* lengths)
{
    m_program = OpenCL::clCreateProgramWithSource(context, count, strings,
                                                  lengths, &m_error);
    CLV(m_error);
...
bool CLProgram::CreateProgramWithSource(cl_context context, const char* src,
                                        const size_t length)
{
...
{
    m_error = OpenCL::clBuildProgram(m_program, 1, &device_id, nullptr, nullptr,
                                     nullptr);
    CLV(m_error);
...
bool CLProgram::BuildProgram(cl_device_id* device_ids, cl_uint dev_count,
                             const char* options)
{
    m_error = OpenCL::clBuildProgram(m_program, dev_count, device_ids, nullptr,
                                     nullptr, nullptr);
    CLV(m_error);
    return (m_error == CL_SUCCESS);
}
...
bool CLKernel::SetKernelArg(cl_uint arg_index, size_t arg_size,
                            const void* arg_value)
{
...
bool CLKernel::GetKernelWorkGroupInfo(cl_device_id device,
                                      CLKernelWorkGroupInfo& info)
{
    OpenCL::clGetKernelWorkGroupInfo(
        m_kernel, device, CL_KERNEL_WORK_GROUP_SIZE, sizeof(info.WorkGroupSize),
        &info.WorkGroupSize, nullptr);
    OpenCL::clGetKernelWorkGroupInfo(
        m_kernel, device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE,
        sizeof(info.PreferredWorkGroupSizeMultiple),
        &info.PreferredWorkGroupSizeMultiple, nullptr);
    OpenCL::clGetKernelWorkGroupInfo(m_kernel, device, CL_KERNEL_LOCAL_MEM_SIZE,
                                     sizeof(info.LocalMemSize),
                                     &info.LocalMemSize, nullptr);
    OpenCL::clGetKernelWorkGroupInfo(
        m_kernel, device, CL_KERNEL_PRIVATE_MEM_SIZE,
        sizeof(info.PrivateMemSize), &info.PrivateMemSize, nullptr);
    m_error = OpenCL::clGetKernelWorkGroupInfo(
        m_kernel, device, CL_KERNEL_COMPILE_WORK_GROUP_SIZE,
...
bool CLMemObject::CreateBuffer(cl_context context, cl_mem_flags flags,
                               size_t size, void* host_ptr)
{
    m_mem_object =
        OpenCL::clCreateBuffer(context, flags, size, host_ptr, &m_error);
...
{
    m_mem_object = OpenCL::clCreateImage2D(context, opts, format, width, height,
                                           row_pitch, data, &m_error);
...
bool CLCommandQueue::Create(cl_context context, cl_device_id device,
                            cl_command_queue_properties properties)
{
    m_queue =
        OpenCL::clCreateCommandQueue(context, device, properties, &m_error);
...
{
    m_error = OpenCL::clEnqueueNDRangeKernel(
...
{
    m_error = OpenCL::clEnqueueReadBuffer(
        m_queue, buffer, blocking_read, offset, size, ptr, 0, nullptr, nullptr);
...
{
    m_error =
        OpenCL::clEnqueueWriteBuffer(m_queue, buffer, blocking_write, offset,
                                     size, ptr, 0, nullptr, nullptr);
...
bool CLCommandQueue::EnqueueReadImage(cl_mem image, cl_bool blocking,
                                      ...
                                      size_t slice_pitch, void* ptr)
{
    m_error = OpenCL::clEnqueueReadImage(m_queue, image, blocking, origin,
                                         region, row_pitch, slice_pitch, ptr, 0,
                                         nullptr, nullptr);
...
bool CLCommandQueue::EnqueueWriteImage(cl_mem image, cl_bool blocking_write,
                                       const size_t* origin,
                                       const size_t* region,
                                       ...
                                       const void* ptr)
{
    m_error = OpenCL::clEnqueueWriteImage(
        m_queue, image, blocking_write, origin, region, input_row_pitch,
        input_slice_pitch, ptr, 0, nullptr, nullptr);
