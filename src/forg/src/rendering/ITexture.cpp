...
ITexture* ITexture::FromFile(IRenderDevice* device, const char* srcFile)
{
    ...
    {
        tex = device->CreateTexture(image.GetWidth(), image.GetHeight(), 0, 0,
                                    Format::A8R8G8B8, Pool_Managed);
    ...
    x /= m_width - 1.0f;
    y /= m_height - 1.0f;
    ba[i] = ((int)(256.0f*noise_gen.noise2d(xstart + 7.0f * x, ystart + 3.0f *
    y)))%256; ba[i+1] = ba[i];//255-ba[i]; ba[i+2] = ba[i];//(i/m_width)%256;
    }*/
