...
XTemplatesMgr::~XTemplatesMgr()
{
    ...
    for (XTemplateVectorI iter = m_templates.begin(); iter != m_templates.end();
         ++iter)
    {
        delete *iter;
    }

    for (XStringTemplateMapI iter = m_std_string_map.begin();
         iter != m_std_string_map.end(); ++iter)
    {
        delete iter->second;
    }
}
...
int XTemplatesMgr::ReadTemplateParts(xreader& treader, XTemplate* tmpl)
{
    ...
    if ((rval = ReadTemplateMembersList(treader, tmpl)) > 0)
        return 1;
    if (treader.ReadToken() != EToken::TOKEN_OBRACKET)
    {
        treader.UnreadToken();

        return (rval > 0); // jesli nie ma EToken::TOKEN_OBRACKET to musi byc
                           // template_members_list
    }

    if (ReadTemplateOptionInfo(treader, tmpl) > 0)
        return 1;
    if (treader.ReadToken() != EToken::TOKEN_CBRACKET)
        return 1;
...
int XTemplatesMgr::ReadTemplateMembers(xreader& treader,
                                       XTemplateMember** member)
{
    int rval;
...
int XTemplatesMgr::ReadTemplatePrimitive(xreader& treader,
                                         XTemplateMember** member)
{
    xstring optional_name;
    ETemplatePrimitiveType::TYPE primitive_type =
        ETemplatePrimitiveType::Unknown;
    int rval;
    ...
    if (treader.ReadToken() != EToken::TOKEN_SEMICOLON)
        return 1;
...
{
    // array                 : EToken::TOKEN_ARRAY array_data_type name
    // dimension_list
    // EToken::TOKEN_SEMICOLON int rval;
    ...
    if (ReadTemplateArrayDataType(treader, aptr_array.get()))
        return 1;
    if (treader.ReadName(aname))
        return 1;
    if (ReadTemplateArrayDimensionList(treader, aptr_array.get()))
        return 1;
    if (treader.ReadToken() != EToken::TOKEN_SEMICOLON)
        return 1;
...
int XTemplatesMgr::ReadTemplateArrayDataType(xreader& treader,
                                             XTemplateArray* arr)
{
    int rval;
...
int XTemplatesMgr::ReadTemplateArrayDimensionList(xreader& treader,
                                                  XTemplateArray* arr)
{
...
int XTemplatesMgr::ReadTemplateArrayDimension(xreader& treader,
                                              XTemplateArray* arr)
{
    if (treader.ReadToken() != EToken::TOKEN_OBRACKET)
    {
    ...
    if (ReadTemplateArrayDimensionSize(treader, arr))
        return 1;
    if (treader.ReadToken() != EToken::TOKEN_CBRACKET)
        return 1;
...
int XTemplatesMgr::ReadTemplateArrayDimensionSize(xreader& treader,
                                                  XTemplateArray* arr)
{
    int ivalue = 0;
    xstring name;
...
int XTemplatesMgr::ReadTemplateReference(xreader& treader,
                                         XTemplateMember** member)
{
    xstring name;
    xstring optional_name;
    ...
    if (rval == 0)
    {
        if (treader.ReadName(optional_name) > 0)
            return 1;
        if (treader.ReadToken() != EToken::TOKEN_SEMICOLON /*YT_SEMICOLON*/)
            return 1;
...
int XTemplatesMgr::ReadTemplateElipsis(xreader& treader, XTemplate* tmpl)
{
    ...
    if (treader.ReadToken() != EToken::TOKEN_DOT)
        return 1;
    if (treader.ReadToken() != EToken::TOKEN_DOT)
        return 1;
...
int XTemplatesMgr::ReadTemplateOptionPart(xreader& treader, xstring& out_name,
                                          xguid& out_guid)
{
