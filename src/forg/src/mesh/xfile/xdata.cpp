...
int XDataObject::Load(xreader& reader, XTemplatesMgr& tmpl_mgr)
{
    ...
    if (EXECUTE_ASSERT(reader.ReadName(m_optional_name) > 0))
        rval = 1;
    ...
    if (EXECUTE_ASSERT(reader.ReadToken() != EToken::TOKEN_OBRACE))
        rval = 1;
    if (EXECUTE_ASSERT(reader.ReadGUID(m_optional_guid) > 0))
        rval = 1;
    if (EXECUTE_ASSERT(ReadMembers(reader, tmpl_mgr)))
        rval = 1;
    if (EXECUTE_ASSERT(reader.ReadToken() != EToken::TOKEN_CBRACE))
        rval = 1;
        ...
        if (field->IsPrimitive())
        {
            EXECUTE_ASSERT(
                ReadPrimitive(reader,
                              static_cast<const XTemplatePrimitive*>(field)
                                  ->GetPrimitiveType()) != 0);
        }
        else
            ...
            {
                const xstring& type =
                    static_cast<const XTemplateReference*>(field)
                        ->GetTemplateName();
                const xstring name =
                    static_cast<const XTemplateReference*>(field)->GetName();

                rval =
                    ReadSubObject(reader, tmpl_mgr, type, name, xguid::Empty);
            }
            else
                // array of primitives or objects
                if (field->IsArray())
                {
                    EXECUTE_ASSERT(ReadArray(reader, tmpl_mgr,
                                             static_cast<const XTemplateArray*>(
                                                 field)) > 0);
                }
                else
                {
...
int XDataObject::ReadSubObject(xreader& reader, XTemplatesMgr& tmpl_mgr,
                           ...
                           const XTemplateArray* xarray)
{
    ...
    else
    {
        ...
        if (idx < m_template->GetMembersCount() && idx < m_subdata.size())
        {
            ...
            {
                DBG_TRACE_ERR("[XDataObject::ReadArray] Only integer array "
...
{
    // data_reference        : EToken::TOKEN_OBRACE name optional_class_id
    // EToken::TOKEN_CBRACE
    ...
    if (treader.ReadName(name))
        return 1;
    if (treader.ReadGUID(guid) > 0)
        return 1;
...
void XDataObject::ToByteArray(void* buffer, uint buffer_size) const
{
    ...
    for (XDataVectorCI iter = m_subdata.begin();
         (bleft > 0) && (iter != m_subdata.end()); ++iter)
    {
...
XDataIdentifier::XDataIdentifier() { m_bPrimitive = false; }
...
void XDataIntegerList::ToByteArray(void* buffer, uint buffer_size) const
{
    ...
    /*
            int i = 0;
            for (IntegerList::const_iterator it=m_aIntegers.begin(); (bsize>0)
       && (it != m_aIntegers.end()); ++it, bsize--)
            {
                ints[i] = *it;
...
void XDataFloatList::ToByteArray(void* buffer, uint buffer_size) const
{
    ...
    /*
            ...
            int i = 0;
            for (FloatList::const_iterator it=m_aFloats.begin(); (bsize>0) &&
       (it != m_aFloats.end()); ++it, bsize--)
            {
                fls[i] = *it;
