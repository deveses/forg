...
template <class T> class basic_string
{
  ...
  public:
    ...
    ~basic_string() { deallocate(m_buffer); }

    const char& operator[](size_type pos) const { return m_buffer[pos]; }

    char& operator[](size_type pos) { return m_buffer[pos]; }
    ...
    basic_string& append(const basic_string& _str, size_type _pos = 0,
                         size_type _num = npos)
    {
        if (_pos >= _str.m_length)
            _pos = 0;
        if (_num == npos)
            _num = _str.m_length - _pos;

        if (_num > 0 && (_num + _pos <= _str.m_length))
        {
            reserve(m_length + _num + 1);

            copy(m_buffer + m_length, _str.m_buffer + _pos,
                 _num * sizeof(char_type));
  ...
  private:
    T* allocate(size_type _size) { return new T[_size]; }

    void deallocate(T* _mem) { delete[] _mem; }
