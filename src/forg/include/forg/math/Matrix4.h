...
struct FORG_API Matrix4
{
    ...
    void Decompose(Vector3* _scale, Quaternion* _rotation,
                   Vector3* _translation) const;
    // Static

    static Matrix4& RotationAxis(Matrix4& mOut, const Vector3& vAxis,
                                 float fAngle);
    ...
    static Matrix4& LookAtLH(Matrix4& mOut, const Vector3& cameraPosition,
                             const Vector3& cameraTarget,
                             const Vector3& cameraUpVector);
    static Matrix4& LookAtRH(Matrix4& mOut, const Vector3& cameraPosition,
                             const Vector3& cameraTarget,
                             const Vector3& cameraUpVector);
    static Matrix4& PerspectiveFovLH(Matrix4& mOut, float fieldOfViewY,
                                     float aspectRatio, float znearPlane,
                                     float zfarPlane);
    static Matrix4& PerspectiveFovRH(Matrix4& mOut, float fieldOfViewY,
                                     float aspectRatio, float znearPlane,
                                     float zfarPlane);
    static Matrix4& OrthoLH(Matrix4& mOut, float width, float height,
                            float znearPlane, float zfarPlane);
    static Matrix4& OrthoRH(Matrix4& mOut, float width, float height,
                            float znearPlane, float zfarPlane);
    static Matrix4& OrthoOffCenterLH(Matrix4& mOut, float xMin, float xMax,
                                     float yMin, float yMax, float znearPlane,
                                     float zfarPlane);
    static Matrix4& OrthoOffCenterRH(Matrix4& mOut, float xMin, float xMax,
                                     float yMin, float yMax, float znearPlane,
                                     float zfarPlane);
    ...
    /// Determines the product of two matrices.
    /**
     * The result represents the transformation M1 (left) followed by the
     * transformation M2 (right) (Out = M1 * M2).
     */
    static Matrix4& Multiply(Matrix4& mOut, const Matrix4& left,
                             const Matrix4& right);

    static Matrix4& Inverse(Matrix4& mOut, const Matrix4& src,
                            float* pDeterminant);
};
