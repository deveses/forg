...
class IRenderDevice : public core::RefCounter
{
  ...
  public:
    ...
    /**
     * Clears one or more surfaces such as a render target, multiple render
     * targets, a stencil buffer, and a depth buffer.
     * @param flags
     * Combination of one or more ClearFlags flags that specify the surface(s)
     * that will be cleared.
     * @param color
     * Clear a render target to this ARGB color.
     * @param zdepth
     * Clear the depth buffer to this new z value which ranges from 0 to 1. See
     * remarks.
     * @param stencil
     * Clear the stencil buffer to this new value which ranges from 0 to 2n - 1
    ...
    /// Create a vertex shader declaration from the device and the vertex
    /// elements.
    /**
     * Create a vertex shader declaration from the device and the vertex
     * elements.
    ...
    CreateVertexDeclaration(const VertexElement* pVertexElements) = 0;
     ...
     * However, if usage is desired, use a combination of one or more Usage
     * constants. It is good practice to match the usage parameter in
     * CreateVertexBuffer with the behavior flags in CreateDevice.
     * @param pool
     * Member of the Pool enumerated type, describing a valid memory class into
     * which to place the resource. Do not set to Pool_Scratch.
     * @return Pointer to an IVertexBuffer interface, representing the created
     ...
     * @param usage
     * Usage can be 0, which indicates no usage value. However, if usage is
     * desired, use a combination of one or more Usage constants. It is good
     * practice to match the usage parameter in CreateIndexBuffer with the
     * behavior flags in CreateDevice.
     ...
     * array of vertices.
     * @param primitiveType
     * Member of the PrimitiveType enumerated type, describing the type of
     * primitive to render.
     * @param baseVertex
     * Offset from the start of the vertex buffer to the first vertex.
     * BaseVertexIndex is a value that's effectively added to every VB Index
     * stored in the index buffer
     * @param minVertexIndex
     * Minimum vertex index for vertices used during this call. This is a zero
     * based index relative to baseVertex.
     * @param numVertices
     * Number of vertices used during this call. The first vertex is located at
     * index: baseVertex + minVertexIndex.
     * @param startIndex
     * Index of the first index to use when accesssing the vertex buffer.
     ...
     * of the primitive count and the primitive type.
     * @return If the method succeeds, the return value is FORG_OK.
     */
    ...
    /// Renders the specified geometric primitive with data specified by a user
    /// memory pointer.
    /**
     * Renders the specified geometric primitive with data specified by a user
     * memory pointer.
     ...
     * @param sixteenBitIndices
     * Set to true to indicate 16-bit indices. Set to false if to indicate
     * 32-bit indices.
     * @param vertexStreamZeroData
     * User memory pointer to the vertex data. The vertex data must be in stream
     * 0.
     ...
     * Sets a single device transformation-related state.
     * @param state
     * Device-state variable that is being modified. This parameter can be any
     * member of the TransformType enumerated type.
     * @param matrix
     * Reference to a Matrix4 structure that modifies the current
     * transformation.
     */
     ...
     * Sets a Vertex Declaration.
     * @param pDecl
     * Pointer to an VertexDeclaration object, which contains the vertex
     * declaration.
     * @return If the method succeeds, the return value is FORG_OK.
     ...
     * Binds a vertex buffer to a device data stream.
     * @param streamNumber
     * Specifies the data stream, in the range from 0 to the maximum number of
     * streams -1.
     * @param streamData
     * Pointer to an IVertexBuffer interface, representing the vertex buffer to
     * bind to the specified data stream.
     * @param offsetInBytes
     * Offset from the beginning of the stream to the beginning of the vertex
     * data, in bytes.
     * @param stride
    ...
    virtual int SetStreamSource(int streamNumber, IVertexBuffer* streamData,
                                int offsetInBytes, int stride) = 0;

    /// Sets index data.
    /**
     * Sets index data.
     * @param pIndexData
     * Pointer to an IIndexBuffer interface, representing the index data to be
     * set.
     * @return If the method succeeds, the return value is FORG_OK.
     * @see IIndexBuffer
     */
    virtual int SetIndices(IIndexBuffer* pIndexData) = 0;

    /// Sets the viewport parameters for the device.
    /**
     * Sets the viewport parameters for the device.
     * @param X
     * Pixel coordinate of the upper-left corner of the viewport on the
     * render-target surface. Unless you want to render to a subset of the
     * surface, this member can be set to 0.
     * @param Y
     * Pixel coordinate of the upper-left corner of the viewport on the
     * render-target surface. Unless you want to render to a subset of the
     * surface, this member can be set to 0.
     * @param Width
     * Width dimension of the clip volume, in pixels. Unless you are rendering
     * only to a subset of the surface, this member should be set to the width
     * dimension of the render-target surface.
     * @param Height
     * Height dimension of the clip volume, in pixels. Unless you are rendering
     * only to a subset of the surface, this member should be set to the height
     * dimension of the render-target surface.
     * @param MinZ
     * Together with MaxZ, value describing the range of depth values into which
     * a scene is to be rendered, the minimum and maximum values of the clip
     * volume. Most applications set this value to 0.0. Clipping is performed
     * after applying the projection matrix.
     * @param MaxZ
     * Together with MinZ, value describing the range of depth values into which
     * a scene is to be rendered, the minimum and maximum values of the clip
     * volume. Most applications set this value to 1.0. Clipping is performed
     * after applying the projection matrix.
     * @return If the method succeeds, the return value is FORG_OK.
     */
     ...
     * Retrieves the viewport parameters currently set for the device.
     * @param viewport
     * [out] Pointer to a Viewport structure, representing the returned viewport
     * parameters
     */
     ...
     * @param state
     * Device state variable that is being modified. This parameter can be any
     * member of the RenderStates enumerated type.
     * @param value
     * New value for the device render state to be set. The meaning of this
     * parameter is dependent on the value specified for State. For example, if
     * State were RenderStates_ShadeMode, the second parameter would be one
     * member of the ShadeMode enumerated type.
     * @return If the method succeeds, the return value is FORG_OK.
     * @see RenderStates
     */
