...
{
public:
    HttpControlServer(const std::string& bindAddr, int port, CommandQueue& queue);
    ~HttpControlServer();

    /// Binds and listens, then spawns the accept thread. Returns false on failure.
    bool Start();

    /// Stops the accept thread and closes the socket. Safe to call more than once.
    void Stop();
