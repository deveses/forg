...
enum
{
    ...
    Point = 1,
    /**
     * Light is a spotlight source. This light is like a point light, except
     * that the illumination is limited to a cone. This light type has a
     * direction and several other parameters that determine the shape of the
     * cone it produces.
     */
    Spot = 2,
    /// Light is a directional light source. This is equivalent to using a point
    /// light source at an infinite distance.
    Directional = 3,
};
};
...
enum EType
{
    ...
    X8R8G8B8 = 0x16,
    /// A 16-bit RGB pixel format that uses 5 bits for red, 6 bits for green,
    /// and 5 bits for blue.
    R5G6B5 = 0x17,
    /// 8-bit alpha only.
    A8 = 0x1c,
...
enum
{
     ...
     * The transforms currently set to the device are used to transform the
     * sprites when the batched sprites are drawn. If this flag is not
     * specified, then world, view, and projection transforms are modified so
     * that sprites are drawn in screen-space coordinates.
     */
    ObjectSpace = 4,
    /// Each sprite will be rotated about its center so that it is facing the
    /// viewer
    Billboard = 8,
