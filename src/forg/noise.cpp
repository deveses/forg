...
float lerp2(float a, float b, float t) { return a * (1.0f - t) + t * b; }
...
int CloudExpCurve(int v, int CloudCover /*0-255*/,
                  float CloudSharpness /*<=1.0*/)
{
    int c = v - CloudCover;
    if (c < 0)
        c = 0;
...
PerlinNoise::PerlinNoise() : m_bInitialized(false)
{
...
void PerlinNoise::init()
{
    ...
    for (int i = 0; i < PSIZE; i++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            grad2d_tab[i][j] = (float)((rand() % (PSIZE + PSIZE)) - PSIZE) /
                               PSIZE; // rand in [-1:1]
...
float PerlinNoise::grad_noise2d(float x, float y)
{
    ...
    for (int i = 0; i < m_nOctavesCount; i++)
    {
        ...
        total =
            total + GradientNoise_2D(x * frequency, y * frequency) * amplitude;
    }
...
float PerlinNoise::int_noise(int x)
{
    x = (x << 13) ^ x;
    return (1.0f -
            float((x * (x * x * 15731 + 789221) + 1376312589) & 0x7fffffff) /
                1073741824.0f);
}
...
float PerlinNoise::SmoothNoise_2D(float x, float y)
{
    float corners = (Noise2D(x - 1, y - 1) + Noise2D(x + 1, y - 1) +
                    ...
                    16;
    float sides = (Noise2D(x - 1, y) + Noise2D(x + 1, y) + Noise2D(x, y - 1) +
                   Noise2D(x, y + 1)) /
                  8;
...
float PerlinNoise::PerlinNoise_2D(float x, float y)
{
    ...
    for (int i = 0; i < m_nOctavesCount; i++)
    {
        ...
        total = total +
                InterpolatedNoise_2D(x * frequency, y * frequency) * amplitude;
    }
