...
namespace forg
{
    ...
    int SWRenderDevice::Initialize(uint _width, uint _height)
    {
        ...
        */
        ...
        OpenCL::CLContext context;
        context.Create(platform->GetPlatformId(), device->GetDeviceId());
        ...
        prog.CreateProgramWithSource(context.GetContext(), kernel_src.data(), kernel_src.size());
        prog.BuildProgram(device->GetDeviceId());

        OpenCL::CLKernel kernel;
        kernel.Create(prog.GetProgram(), "SAXPY");


        DBG_MSG("Prepare data...\n");
        // Prepare some test data
        static const size_t testDataSize = 1 << 10;
        forg::core::vector<float> a(testDataSize), b(testDataSize);
        for (int i = 0; i < testDataSize; ++i) {
            a[i] = static_cast<float> (23 ^ i);
            b[i] = static_cast<float> (42 ^ i);
        }

        OpenCL::CLMemObject aBuffer;
        aBuffer.CreateBuffer(context.GetContext(), CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
            sizeof (float)* (testDataSize), a.data());

        OpenCL::CLMemObject bBuffer;
        bBuffer.CreateBuffer(context.GetContext(), CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
            sizeof (float)* (testDataSize), b.data());

        OpenCL::CLCommandQueue queue;
        queue.Create(context.GetContext(), device->GetDeviceId(), 0);

        kernel.SetKernelArg(0, aBuffer);
        kernel.SetKernelArg(1, bBuffer);
        static const float two = 2.0f;
        kernel.SetKernelArg(2, sizeof (float), &two);

        DBG_MSG("Enqueue kernel...\n");
        job_timer.Start();
        const size_t globalWorkSize[] = { testDataSize, 0, 0 };
        queue.EnqueueNDRangeKernel(kernel.GetKernel(), 1,
            nullptr, globalWorkSize, nullptr);

        DBG_MSG("Enqueue buffer...\n");
        // Get the results back to the host

        queue.EnqueueReadBuffer(bBuffer.GetMemObject(), CL_TRUE, 0,
            sizeof (float)* testDataSize,
            b.data());

        forg::uint64 duration = 0;
        job_timer.GetDurationInMs(duration);
        DBG_MSG("cuda kernel time: %dms\n", duration);

        job_timer.Start();
        kernel_test(a.data(), b.data(), 2.0f, testDataSize);
        job_timer.GetDurationInMs(duration);
        DBG_MSG("cpu kernel time: %dms\n", duration);
