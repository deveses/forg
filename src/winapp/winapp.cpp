...
BOOL CWinApp::InitApplication()
{
    ...
    if (xml_doc)
    {
        if (forg::script::xml::XMLNode* xml_node =
                xml_doc->FindNode("renderer"))
        {
            if (forg::script::xml::XMLNode* xml_att =
                    xml_node->FindAttribute("driver"))
            {
        ...
        {
            if (forg::script::xml::XMLNode* xml_att =
                    xml_node->FindAttribute("width"))
            {
                winWidth = atoi(xml_att->GetContent().c_str());
            }

            if (forg::script::xml::XMLNode* xml_att =
                    xml_node->FindAttribute("height"))
            {
                winHeight = atoi(xml_att->GetContent().c_str());
            }

            if (forg::script::xml::XMLNode* xml_att =
                    xml_node->FindAttribute("posx"))
            {
                winX = atoi(xml_att->GetContent().c_str());
            }

            if (forg::script::xml::XMLNode* xml_att =
                    xml_node->FindAttribute("posy"))
            {
    ...
    // if (m_winMain.Create("EWindow","3Ditor",CW_USEDEFAULT
    // ,SW_MAXIMIZE,480,460,WS_SIZEBOX|WS_MAXIMIZEBOX|WS_MAXIMIZE|WS_TILEDWINDOW|WS_CLIPCHILDREN|WS_CLIPSIBLINGS
    // ))
