...
// All Metal/ObjC objects live in this opaque struct (defined in the .mm) so this
// header is includable from plain C++ (e.g. the factory in MetalRenderer.cpp).
struct MetalImpl;

/// Hardware render device implemented on Apple Metal.
/**
* Implements the subset of IRenderDevice that the macapp demo exercises, hosting
* a CAMetalLayer in the NSView passed as HWIN. See docs/metal-renderer-plan.md.
*/
...
public:
    ...
    // Brings up the Metal device, command queue, shaders and presentation layer.
...
public:
    ...
    virtual LPVERTEXDECLARATION CreateVertexDeclaration(const VertexElement* pVertexElements);
    virtual LPVERTEXBUFFER CreateVertexBuffer(uint length, uint usage, uint pool);
    virtual LPINDEXBUFFER CreateIndexBuffer(uint length, uint usage, bool sixteenBitIndices, uint pool);
    virtual LPTEXTURE CreateTexture(uint Width, uint Height, uint Levels, uint Usage, uint Format, uint Pool);
    virtual LPTEXTURE CreateTextureFromFile(const char* filename, uint Width, uint Height, uint Levels, uint Usage, uint Format, uint Pool);

    virtual int DrawIndexedPrimitive(PrimitiveType primitiveType, int baseVertex, int minVertexIndex, int numVertices, int startIndex, int primCount);
    virtual int DrawIndexedUserPrimitives(PrimitiveType primitiveType, uint minVertexIndex, uint numVertexIndices, uint primitiveCount, const void* indexData, bool sixteenBitIndices, const void* vertexStreamZeroData, uint vertexStreamZeroStride);
    ...
    virtual int SetStreamSource(int streamNumber, IVertexBuffer* streamData, int offsetInBytes, int stride);
