...
namespace forg
{
    ...
    struct int2
    {
        ...
        int2() restrict(amp, cpu) {}
        int2(int _x, int _y) restrict(amp, cpu) { x = _x; y = _y; }

        int2 operator & (int i) restrict(amp, cpu)
        {
            return int2(x&i, y&i);
        }

        void operator &= (int i) restrict(amp, cpu)
        {
            x &= i;
            y &= i;
        }
    ...
    int2 operator - (int2 v1, int2 v2) restrict(amp, cpu)
    {
        return int2(v1.x + v2.x, v1.y + v2.y);
    }

    int2 operator + (int2 v, int i) restrict(amp, cpu)
    {
        return int2(v.x + i, v.y + i);
    }
    ...
    struct float2
    {
        ...
        float2() restrict(amp, cpu) {}
        float2(float _x, float _y) restrict (amp, cpu) { x = _x; y = _y; }
    };

    float2 operator * (float f, float2 f2) restrict(amp, cpu)
    {
    ...
    struct float4
    {
        ...
        float4() restrict(amp, cpu) {}
        float4(float _x, float _y, float _z, float _w) restrict(amp, cpu) { x = _x; y = _y; z = _z; w = _w; }

        float2 xy() const restrict(amp, cpu) { return float2(x, y); }
    };

    float dot(float2 a, float2 b) restrict(amp, cpu)
    {
        return a.x*b.x + a.y*b.y;
    }

    float linear_eq(float2 A, float C, float2 x) restrict(amp, cpu)
    {
    ...
    float det(float2 col1, float2 col2) restrict(amp, cpu)
    {
    ...
    bool SWRenderDevice::InitializeCL()
    {
        ...
        {
            DBG_MSG("AMP device %d\n", i);
            DBG_MSG(L"   device description: %s\n", accs[i].description.c_str());
            DBG_MSG(L"   device path: %s\n", accs[i].device_path.c_str());
            DBG_MSG(L"   dedicated memory: %d\n", accs[i].dedicated_memory);
            DBG_MSG("   CPU shared memory: %s\n", accs[i].supports_cpu_shared_memory ? "true" : "false");
            DBG_MSG("   double precision: %s\n", accs[i].supports_double_precision ? "true" : "false");
            DBG_MSG("   limited double precision: %s\n", accs[i].supports_limited_double_precision ? "true" : "false");
        }
            ...
            u32 vertexStreamZeroStride)
    {
        ...
        for (u32 i=0; i<primitiveCount; i++)
        {
            ...
            {
                DrawTriangleArrayAMP(vs_batch, tri_count, vertex_usage);
    ...
    struct TraingleInterpolatorAMP
    {
        float2 lineAB01;
        float2 lineAB12;
        float2 lineAB20;
        ...
        void Initialize(const float2& pos0, const float2& pos1, const float2& pos2) restrict(amp)
        {
            lineAB01 = float2(pos0.y - pos1.y, pos1.x - pos0.x);
            lineAB12 = float2(pos1.y - pos2.y, pos2.x - pos1.x);
            lineAB20 = float2(pos2.y - pos0.y, pos0.x - pos2.x);
    ...
    class DrawBlockKernel
    {
    public:
        DrawBlockKernel(const uint2& dim, const concurrency::array_view<u32, 2>&  fbuffer, float* zbuffer, VSOutput* vertices, u32 num_triangles, int usage)
            : m_dim(dim)
            , m_num_tris(num_triangles)
            , m_cbuffer(fbuffer)
        ...
        static int2 convert_int2(float2 v) restrict(cpu, amp)
        {
            int2 r;

            r.x = (int)concurrency::fast_math::truncf(v.x);
            r.y = (int)concurrency::fast_math::truncf(v.y);

            return r;
        }

        static int2 min3_int2(int2 v1, int2 v2, int2 v3) restrict(cpu, amp)
        {
            int2 r;

            r.x = concurrency::fast_math::fminf(concurrency::fast_math::fminf(v1.x, v2.x), v3.x);
            r.y = concurrency::fast_math::fminf(concurrency::fast_math::fminf(v1.y, v2.y), v3.y);

            return r;
        }

        static int2 max3_int2(int2 v1, int2 v2, int2 v3) restrict(cpu, amp)
        {
            int2 r;

            r.x = concurrency::fast_math::fmaxf(concurrency::fast_math::fmaxf(v1.x, v2.x), v3.x);
            r.y = concurrency::fast_math::fmaxf(concurrency::fast_math::fmaxf(v1.y, v2.y), v3.y);

            return r;
        }

        static int2 max_int2(int2 v1, int2 v2) restrict(cpu, amp)
        {
            int2 r;

            r.x = concurrency::fast_math::fmaxf(v1.x, v2.x);
            r.y = concurrency::fast_math::fmaxf(v1.y, v2.y);

            return r;
        }

        static int2 min_int2(int2 v1, int2 v2) restrict(cpu, amp)
        {
            int2 r;

            r.x = concurrency::fast_math::fminf(v1.x, v2.x);
            r.y = concurrency::fast_math::fminf(v1.y, v2.y);

            return r;
        }

        static int det_int2(int2 col1, int2 col2) restrict(amp)
        {
            return col1.x*col2.y - col2.x*col1.y;
        }
        ...
        {
            TraingleInterpolatorAMP interpolator;

            int sx = idx[0];
            int sy = idx[1];
            int2 spos(sx, sy);

            for (u32 t = 0; t<m_num_tris; t++)
            {
                u32 v0 = t * 3 + 0;
                u32 v1 = t * 3 + 1;
                u32 v2 = t * 3 + 2;

                const float2 pos0 = m_vertices[v0].position.xy();
                const float2 pos1 = m_vertices[v1].position.xy();
                const float2 pos2 = m_vertices[v2].position.xy();
                ...
                bounds_min = max_int2(bounds_min, int2(0, 0));
                bounds_max = min_int2(bounds_max, int2(m_dim.x, m_dim.y));
    ...
    private:
        uint2 m_dim;
        u32 m_num_tris;
        concurrency::array_view<AMPVSOutput, 1> m_vertices;
        concurrency::array_view<u32, 2> m_cbuffer;
        concurrency::array_view<float, 2> m_zbuffer;
    };

    void SWRenderDevice::DrawTriangleArrayAMP(VSOutput* vertices, u32 num_triangles, int usage)
    {
        concurrency::array_view<u32, 2> iter(m_width, m_height, m_frame_buffer);
        iter.discard_data();

        concurrency::parallel_for_each(iter.extent, DrawBlockKernel(uint2(m_width, m_height), iter, m_depth_buffer, vertices, num_triangles, usage));
    }
