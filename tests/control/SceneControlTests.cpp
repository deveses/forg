...
struct Scene
{
    ...
    Scene() : light(), clearColor(0.0f, 0.0f, 0.0f), meshTm(forg::Matrix4::Identity) {}
...
TEST_CASE("camera.place sets absolute position and target", "[control]")
{
    ...
    std::string r = DispatchCommand(ctx,
...
TEST_CASE("clear.color updates the clear color", "[control]")
{
    ...
    std::string r = DispatchCommand(ctx,
        CommandFromRequest("/clear/color", "r=0.25&g=0.5&b=0.75"));
...
TEST_CASE("light.set updates only the supplied fields", "[control]")
{
    ...
    std::string r = DispatchCommand(ctx,
        CommandFromRequest("/light/set", "px=3&dr=0.5&range=42"));
...
TEST_CASE("state reports the current scene as JSON", "[control]")
{
    Scene scene;
    SceneControlContext ctx = scene.context();
    DispatchCommand(ctx, CommandFromRequest("/camera/place", "px=1&py=2&pz=3&tx=0&ty=0&tz=0"));
...
TEST_CASE("an unknown verb is rejected", "[control]")
{
    ...
    std::string r = DispatchCommand(ctx, CommandFromRequest("/does/not/exist", ""));
...
TEST_CASE("camera.zoom without fov reports a bad parameter", "[control]")
{
    ...
    std::string r = DispatchCommand(ctx, CommandFromRequest("/camera/zoom", ""));
